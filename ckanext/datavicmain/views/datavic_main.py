...
def historical(package_type: str, package_id: str):
    ...
    return toolkit.render(
        "package/read_historical.html", {"pkg_dict": pkg_dict}
    )
...
def admin_report():
    ...
    if report_type and report_type == "user-email-data":
        ...
        fd.writerow(["Entity type", "Email", "URL"])
...
def toggle_organization_uploads(id: str) -> Response:
    try:
        toolkit.check_access(
            "datavic_toggle_organization_uploads", {}, {"id": "id"}
        )
    except toolkit.NotAuthorized:
        return toolkit.abort(
            403, toolkit._("Not authorized to change uploads for organization")
        )
    ...
    if org := model.Group.get(id):
        try:
            ...
            toolkit.get_action("flakes_flake_create")(
                {"ignore_auth": True},
                {
                    "author_id": None,
                    "name": utils.org_uploads_flake_name(),
                    "data": {org.id: True},
                    "extras": {
                        "datavicmain": {"type": "organization_uploads_list"}
                    },
                },
            )
        else:
            flake["data"].update(
                {org.id: not flake["data"].get(org.id, False)}
            )
            toolkit.get_action("flakes_flake_update")(
                {"ignore_auth": True}, flake
            )
    ...
    for id_ in ids:
        ...
        try:
            ...
            if resource["package_id"] not in pkg_cache:
                pkg_cache[resource["package_id"]] = toolkit.get_action(
                    "package_show"
        ...
        catalog.append(
            {
                ...
                "name": "{}: {}".format(
                    pkg["title"], resource["name"] or "Unnamed"
                ),
                "type": "ckan-item",
                "url": base_url,
                "resourceId": id_,
...
def register_datavicmain_plugin_rules(blueprint):
    blueprint.add_url_rule(
        "/<package_type>/<package_id>/historical", view_func=historical
    )
    ...
    blueprint.add_url_rule(
        "/organization/edit/<id>/toggle-uploads",
        view_func=toggle_organization_uploads,
        methods=["POST"],
    )
